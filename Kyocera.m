...
for j = 1:length(xvals)
    ...
    Gcell = 800; Tcell = 45; Imax = 9.5 * Gcell / 1000;
    sim(simul);
    power = 60 * Vpvout .* Ipvout;
    [m, in] = max(power); % max value and index
    relmiss(j,1) = (m - 191)/191;
    relmiss(j,3) = Ipvout(in)/6.85 - 1;
    relmiss(j,5) = 60 * Vpvout(in)/27.9 - 1;
    
    Gcell = 1000; Tcell = 25; Imax = 9.5 * Gcell / 1000;
    sim(simul);
    power = 60 * Vpvout .* Ipvout;
    [m, in] = max(power); % max value and index
    relmiss(j,2) = m/265 - 1;
    relmiss(j,4) = Ipvout(in)/8.55 - 1;
